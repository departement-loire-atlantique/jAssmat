				...
				if (!validateFields()) {
					...
					if (Util.notEmpty(idInscriptionAM)) {
						...
						if ("mail".equalsIgnoreCase(typeenvoi)) {
							...
							String url =  channel.getUrl() + "espaceperso?token=" + tokenGenerate + "\">" + channel.getUrl() + "espaceperso?token=" + tokenGenerate;
							String lien = "<a href=\"" + url + "</a>";
...
public Double getLatMAM() {
  ...
  public void setOpNext(String opNext) {
    this.opNext = Util.notEmpty(opNext);
  }
  
  public void setOpPrevious(String opPrevious) {
    this.opPrevious = Util.notEmpty(opPrevious);
  }
  
  public void setOpCreate(String opCreate) {
    this.opCreate = Util.notEmpty(opCreate);
  }
