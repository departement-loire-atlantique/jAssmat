 ...
 *
 */
public class MemberRAMDataController extends BasicDataController {
  ...
  public void afterWrite(Data data, int op, Member mbr, Map map) {
   ...
   // Si c'est une création on désactive le comtpe
      if (op == OP_UPDATE) {
        ...
        //Si le groupe RAM est ajouté au membre
        ...
        if(Util.notEmpty(memberData.getGroups())){
           if(Util.notEmpty(oldMember.getGroups())){
             ...
             if(!oldGroupList.contains(groupRAM) && newGroupList.contains(groupRAM)){
  ...
  public static void sendMail(Member memberData){
    ...
    parameters = (String[]) Util.insertArray(parameters, 1, "<a href='" + portletValidepass.getDisplayUrl(channel.getLocale()) + "?portal="+idPortalAccueil+"&idMember="+memberData.getId()+"'>J’active le compte de mon relais</a>");

    AssmatUtil.getMessagePropertiesParameters("ACTIVATION-MEMBER-RAM-MAIL-CONTENT", parameters);   
