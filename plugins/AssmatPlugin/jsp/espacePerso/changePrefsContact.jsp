  ...
  <form method="post" action="<%= ServletUtil.getResourcePath(request) %>" 
    ...
    <%-- Préférence de type d'envoi --%>
        ...
        <div id="form-element-<%= uuid %>" data-name="typeenvoi" class="ds44-form__radio_container ds44-form__container"  data-required="true">
            <p aria-level="2" class="h4-like"><%= glp("jcmsplugin.assmatplugin.inscription.champ.lbl.typeenvoi") %><sup aria-hidden="true">*</sup></p>
            ...
            <div class="ds44-form__container ds44-checkBox-radio_list inbl">
                <input type="radio" name="typeenvoi" value="mail" id="name-radio-form-element-<%= uuid %>-mail" <%if(isCanalMail){ %>checked="checked"<%} %> class="ds44-radio" required aria-describedby="mandatory-message-form-element-<%= uuid %>" /><label id="label-radio-form-element-<%= uuid %>-mail" for="name-radio-form-element-<%= uuid %>-mail" class="ds44-radioLabel"><%=glp("jcmsplugin.assmatplugin.inscription.champ.typeenvoi.value.mail") %></label>
            </div>
            <div class="ds44-form__container ds44-checkBox-radio_list inbl">
                <input type="radio" name="typeenvoi" value="telephone" id="name-radio-form-element-<%= uuid %>-telephone" <%if(isCanalTel){ %>checked="checked"<%} %> class="ds44-radio" required aria-describedby="mandatory-message-form-element-<%= uuid %>" /><label id="label-radio-form-element-<%= uuid %>-telephone" for="name-radio-form-element-<%= uuid %>-telephone" class="ds44-radioLabel"><%=glp("jcmsplugin.assmatplugin.inscription.champ.typeenvoi.value.sms") %></label>
            </div>
        </div>
        <div class="mbl">
            <p>
    ...
    <h3 class="h3-like"><%= glp("jcmsplugin.assmatplugin.espacepro.modifiercoords") %></h3>
		...
		<p class="h5-like" style="padding-bottom: 20px">et/ou</p>
		<%-- Téléphone --%>
		<% uuid = UUID.randomUUID().toString(); %>
		<div class="ds44-form__container">
		    <!--                <p aria-level="2" class="h4-like"> -->
